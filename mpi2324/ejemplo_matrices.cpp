...
void matrix_mult(double *A, double *b, double *c, int rows, int cols) {
    for (int i = 0; i < rows; i++) {
        double tmp = 0;  // Inicializar tmp a cero
...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        matrix_mult(A_local.data(), b_local.data(), c.data(), rows_per_rank_tmp, MATRIX_DIMENSION);

        // Enviar el resultado parcial
        MPI_Gather(c.data(), rows_per_rank, MPI_DOUBLE,
                   nullptr, 0, MPI_DOUBLE,
                   0, MPI_COMM_WORLD);
    }
